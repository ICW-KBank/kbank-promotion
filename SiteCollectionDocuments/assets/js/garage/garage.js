...
function adobeRenderPage(){
    ...
    if(typeof d.adbConfig == "object"){
        ...
        if(d.adbConfig.widget != undefined && d.adbConfig.widget.order.length){
            var _loc = j.getElementById("quick-cash-loans").querySelector(".intro-card-container"),
        ...
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || $(document).width() < 992) {
            ...
            */
            ...
            if ($('.sc-apply-online.d').length) {
                var sc_scrollTop = $(document).scrollTop();
                var quick_cash_loans_s = $('#quick-cash-loans').offset().top;
                var quick_cash_loans_e = $('#quick-cash-loans').offset().top + $('#quick-cash-loans').height();
                ...
                if ((sc_scrollTop >= quick_cash_loans_s && sc_scrollTop <= quick_cash_loans_e) || (sc_scrollTop >= application_details_s && sc_scrollTop <= (application_details_e - (application_details_h / 4))) || (sc_scrollTop >= faqs_s && sc_scrollTop <= faqs_e)) {
...
$(function () {
    ...
    // on initial
    rewidth("#responsive-scroll-01", "#responsive-table-01");
    rewidth("#responsive-scroll-02", "#responsive-table-02");
    rewidth("#responsive-scroll-03", "#responsive-table-03");

    // on windows resizing
    $(window).on("resize", rewidth("#responsive-scroll-01", "#responsive-table-01"));
    $(window).on("resize", rewidth("#responsive-scroll-02", "#responsive-table-02"));
    $(window).on("resize", rewidth("#responsive-scroll-03", "#responsive-table-03"));

    // on table scroll
    bindingScroll("#responsive-scroll-01", "#responsive-table-01");
    bindingScroll("#responsive-scroll-02", "#responsive-table-02");
    bindingScroll("#responsive-scroll-03", "#responsive-table-03");
    ...
    if ($("#rate-table").length) {
        ...
        if (window.location.hash == '#rate-table') {
        ...
        $('.open-rate-table').click(function (e) {
            ...
            $(window).scrollTop($('#general-interest-rate').offset().top - $('.header-top').height(), 0);
        });
    }
});
